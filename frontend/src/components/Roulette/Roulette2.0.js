...
const NewRoulette = ({ setWinnerGame, active, betAmount }) => {
  ...
  const handlePlay = async () => {
    let aux = await Math.floor(Math.random() * 37);
    await active(true);
    await setWinner(aux);
  ...
  useEffect(() => {
    ...
    function Roulette(_canvas, _context) {
      ...
      function drawFrame() {
        ...
        if (_ballSettings.showBall) {
          ...
          _ballSettings.spinTimeElapsed =
            Date.now() - _ballSettings.spinStartTime;
          let f = _ballSettings.spinTimeElapsed / _ballSettings.spinTotalTime;
          f = f > 1 ? 1 : f;
          ctx.rotate((_worldAngle + _ballSettings.finalPosition) * f);
          ctx.beginPath();
          ctx.fillStyle = "#ffffff";
          let vertDeceleration,
            g = _ballSettings.spinTimeElapsed / _ballSettings.spinTotalTime;
          g = g > 1 ? 1 : g;
          if (g < 0.1) {
            vertDeceleration = 1;
          } else {
            vertDeceleration =
              (1 - g) * Math.abs(Math.sin(10 * g * g * _worldAngle));
          }
      ...
      function initBallSpin() {
        ...
        _ballSettings.spinFinalTime =
          _ballSettings.spinStartTime + _ballSettings.spinTotalTime;
      }
      this.dropBall = function (num) {
        ...
        initBallSpin();
        _ballSettings.finalPosition =
          randomBetween(5, 10) * 2 * Math.PI + +slot.position;
        _ballSettings.startPosition = _worldAngle;
        _ballSettings.arcIncrement =
          (_ballSettings.finalPosition - _ballSettings.startPosition) /
      ...
      this.init = function () {
        ...
        _slotText.size =
          (_outerRing.outerRadius - _outerRing.innerRadius) * 0.5;
    ...
    document
      .getElementById("drop-form")
      .addEventListener("submit", function (e) {
        let slot = document.getElementById("drop-input").value;
  ...
  return (
    <div class="wrap">
      ...
      <form class="drop-form" id="drop-form" action="#">
        <input
          ...
          required
        />
        <button onClick={handlePlay} className={classes.button} type="submit">
          Drop Ball
        </button>
      </form>
    </div>
